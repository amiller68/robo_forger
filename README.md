...
<img src="https://github.com/amiller68/robo_forger/blob/31b45fd845ce5a1ae888667ac7faeb9bea0d9af1/scripts/test_images/leaf.png" alt="Maple Leaf Image" width="300"/>
<img src="https://github.com/amiller68/robo_forger/blob/520d4eb5c19f63b976a737f161a9c06c8939f991/ezgif-4-180f11597a.gif" alt="Maple Leaf Gif" width="520"/>
</p>
...
- Computer Vision: The turtlebot is successfully able to take an image, identify line segments within that image, and then convert those segments into points that can be sent to the robot, performing a coordinate transformation to place these points within the range of the robot's arm. The line segments are also ordered such that minimal movement is required between finishing one line and starting the next. We found that implementing this ordering cut the drawing time in half for the maple leaf test, and it significantly improved the quality of the drawing. Since the arm performs less movements, there are fewer opportunities to introduce positioning errors and extra marks from lifting/placing the pen. Below is the maple leaf test without this feature implemented, which can be compared to our demo video:

<p float="left" align="middle">
<img src="https://user-images.githubusercontent.com/38731359/170641325-bbd4946b-540d-4c48-b418-35818c2be5ab.mp4" />
</p>
...
<p float="left" align="middle">
<img src="https://user-images.githubusercontent.com/38731359/170641387-1e65080a-d9d8-4a5c-a739-49bf39ab0be5.mov" />
</p>
...
<p float="left" align="middle">
<img src="https://github.com/amiller68/robo_forger/blob/31b45fd845ce5a1ae888667ac7faeb9bea0d9af1/scripts/test_images/leaf.png" alt="Maple Leaf Image" width="300"/>
<img width="600" src="https://user-images.githubusercontent.com/102747072/170362215-ed82eb43-41f6-467f-81ce-b7586a0356e0.png">
</p>

- Inverse Kinematics: The custom inverse kinematics code is implemented in `inverse_kinematics.py`. Parameters are set up to specify the gripper angle that compensates for the weight of the writing utensil, whether the robot is already holding the writing utensil, and offsets that define how much the arm pushes into the wall when writing, how much it moves off the wall when moving between non-contiguous lines, and a correction term that tells the arm to back off the wall slightly at the top of its drawing area. The constructor for the `RoboForgerIK` class sets up lengths of the OpenMANIPULATOR arm joints, the starting position of the robot, and the appropriate publishers and subscribers. The function `compute_inverse_kinematics` implements a custom 3 DOF IK algorithm which subtracts out the horizontal length of the gripper, computes the yaw angle of the arm, and then projects into the forward/back and up/down plane of the whiteboard to compute standard 2D IK for two revolute joints. Finally, it accounts for rigid sections of the arm perpendicular to the joints, as well as the fact that angles swing out from the vertical, rather than the horizontal. The diagram below demonstrates which pieces of code correspond with which segments of the inverse kinematics algorithm:

<img width="971" alt="Inverse Kinematics" src="https://user-images.githubusercontent.com/38731359/170643792-623150a8-d9e9-4cbb-a264-b6868192e43e.png">

The `move_marker` function moves the marker to a specified (x, y, z) position with a particular number of waypoints by interpolating between the line's endpoints and invoking `compute_inverse_kinematics`, ensuring the computed joint angles are within the ranges of the OpenMANIPULATOR arm before invoking `move_marker_to_pose` to execute the move. It also uses and updates the `curr_pos` value, which is necessary for computing the linear interpolation. The `move_marker_to_pose` function, invoked by `move_marker`, accepts joint angles for the arm, and it computes the gripper joint angle to be perpendicular to wall based on the angles of the other joints, accounting for the weight of the writing implement. It then executes the motion on the arm, with some delay to allow time for the action to finish. Finally, the `recv_point` function is registered as a callback to the `/robo_forger/point` topic, and it begins by computing the distance to the point. If the robot arm is sufficiently close to the point, the move will be skipped, which saves time when switching between lines that intersect at an endpoint. A lift distance and draw distance are computed based on parameters at the top of the file, which define how much the robot should move in and out of the plane of the whiteboard when moving the arm between endpoints and when drawing a line. A coordinate conversion is also made, which scale the coordinates of the points to be located within the robot's drawing area, which was found in testing. The `Point` data consists of a Boolean `start` attribute. If the point received is a starting point, the robot will lift the arm from the board, move it to above the start point, and then place the marker on the board, by invoking `move_marker` three separate times. Otherwise, the point is at the end of the line segment, and the robot will invoke `move_marker` once to draw to that point, with a number of waypoints proportional to the length of the line.
